...
Transkribus (the Text Titan II super-model, via the web app) on three of the
...
$=1.00$), but on the two-column printed English page it read \emph{across} the
columns line by line, scoring \tw{} $=0.51$ --- the same collapse as the pure
detectors, on the layout that looks easiest. Reading order is not something a
strong recognition model can be assumed to get for free; it depends on the layout
segmentation, and even a leading commercial system can miss it on clean
multi-column print.
