...
twentieth-century printed English page. We argue that reading order should be
reported as a first-class metric alongside recognition accuracy, and release the
data, ground truth and a reproducible harness.
\end{abstract}
...
\textbf{A two-tier recommendation.} The fast detector is roughly nine times
quicker per page than kraken, and on single-column material it is already
sufficient. The practical rule the numbers point to is: single column, use a fast
detector; multi-column, either pay for native ordering (kraken) or add an explicit
geometric ordering stage (for example XY-cut~\cite{xycut} or column clustering) on
top of the detector. Speed is real, but it is not free on complex layouts.
...
is enough to establish the effect cleanly but not to rank systems finely; the
natural next steps are more pages, datasets with explicit region and
reading-order ground truth such as cBAD~\cite{cbad} and DIVA-HisDB~\cite{divahisdb},
and a measured geometric ordering stage on top of the
fast detector.
...
layouts. We recommend that reading order be measured and reported as a first-class
metric alongside recognition accuracy, and release the pages, ground truth,
predictions and a reproducible scoring harness so the measurement is easy to
repeat and extend.
